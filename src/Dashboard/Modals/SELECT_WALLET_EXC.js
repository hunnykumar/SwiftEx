...
import darkBlue_1 from "../../../assets/darkBlue.png"
...
export default function SELECT_WALLET_EXC  ({ props, visible, setVisible, setModalVisible }){
  ...
  return (
    ...
    >
      <View style={[style.Body,{backgroundColor:state.THEME.THEME===false?"#fff":"black"}]}>
        ...
        <View style={[style.header,{backgroundColor:state.THEME.THEME===false? "#4CA6EA":"black",borderWidth:0.5,borderBottomColor:state.THEME.THEME===false? "#4CA6EA":"gray"}]}>
    <TouchableOpacity style={style.backButton} onPress={()=>{setVisible(false)}}>
    <Icon name={"left"} type={"antDesign"} size={29} color={"white"}/>
    </TouchableOpacity>
    {Platform.OS==='android'?<Text style={[style.headerText_android,]}>Select Wallet</Text>:<Text style={style.headerText}>Select Wallet</Text>}
    <TouchableOpacity onPress={()=>{setVisible(false),navigation.navigate("Home")}}>
    <Image source={darkBlue_1} style={style.headerImage} />
    </TouchableOpacity>
  </View>  
        <TouchableOpacity
          style={style.Box2}
