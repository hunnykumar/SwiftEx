...
import { View, Text, PanResponder, StyleSheet, Dimensions, Animated, ActivityIndicator, ScrollView, TouchableOpacity } from 'react-native';
...
const CandleStickChart = ({ visible, activeTheme }) => {
  ...
  const loadChartData = async (timeframe) => {
    setisLoading(true);
    const base = StellarSdk.Asset.native();
    ...
    const chartRes = await fetchCandleStick(base, counter, tf.hours, tf.resolution, tf.limit);
...
const handleTouch = (locationX) => {
    ...
    return (
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <Text style={{ color: theme.headingTx, marginTop: 12 }}>
          Loading {selectedTimeframe} chart...
        </Text>
      </View>
    );
  }
  ...
  if (candlestickData.length === 0) {
    return (
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={{ color: theme.headingTx }}>No data available</Text>
      </View>
    );
  }
  ...
  const chartWidth = (CANDLE_WIDTH + CANDLE_GAP) * candlestickData.length;
      ...
      <View style={styles.header}>
        <View style={{ flex: 1 }}>
          <View style={styles.titleRow}>
            <Text style={styles.title}>XLM/USDC</Text>
          </View>
          <View style={styles.ohlcContainer}>
            <Text style={styles.ohlcItem}>
              O: <Text style={[styles.ohlcValue, {color:ohlcColorStyle}]}>{displayData.open.toFixed(7)}</Text>
            </Text>
            <Text style={styles.ohlcItem}>
              H: <Text style={[styles.ohlcValue, {color:ohlcColorStyle}]}>{displayData.high.toFixed(7)}</Text>
            </Text>
            <Text style={styles.ohlcItem}>
              L: <Text style={[styles.ohlcValue, {color:ohlcColorStyle}]}>{displayData.low.toFixed(7)}</Text>
            </Text>
            <Text style={styles.ohlcItem}>
              C: <Text style={[styles.ohlcValue, {color:ohlcColorStyle}]}>{displayData.close.toFixed(7)}</Text>
            </Text>
            <Text style={styles.ohlcItem}>
              Volume (XLM): <Text style={[styles.ohlcValue, {color:ohlcColorStyle}]}>{(displayData.volume / 1000000).toFixed(2)}M</Text>
            </Text>
          </View>
        </View>
